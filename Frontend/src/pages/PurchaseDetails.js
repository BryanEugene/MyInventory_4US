...
function PurchaseDetails() {
  ...
  const [purchase, setAllPurchaseData] = useState([
    {
      _id: "1",
      ProductID: { name: "Sample Product 1" },
    ...
    {
      _id: "2",
      ProductID: { name: "Sample Product 2" },
      QuantityPurchased: 5,
      PurchaseDate: new Date(new Date().setDate(new Date().getDate() - 1)).toLocaleDateString(),
              ...
              {purchase.map((element) => {
                return (
                  <tr key={element._id}>
                    ...
                    <td className="whitespace-nowrap px-4 py-2 text-gray-700">
                      {element.PurchaseDate === new Date().toLocaleDateString()
                        ? "Today"
